...
class XiaoYunQueClient:
    ...
    def submit(
        ...
        retryable_codes = {50429, 50430, 50500, 50501}
        max_retries = 6
        backoff = 10

        for attempt in range(max_retries):
            resp = volc_base.call(
                self._service, "CVSync2AsyncSubmitTask", body, "小云雀"
            )
            code = resp.get("code")
            ...
            if code in retryable_codes and attempt < max_retries - 1:
                print(
                    f"  [可重试错误 code={code}] "
                    f"{resp.get('message', '')}, "
                    f"第 {attempt + 1}/{max_retries} 次重试,等待 {backoff}s..."
                )
                time.sleep(backoff)
        ...
        raise RuntimeError("小云雀提交任务失败:重试次数耗尽")
